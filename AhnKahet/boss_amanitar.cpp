    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if(m_uiBoltTimer < uiDiff)
        {
            if(Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
                m_creature->CastSpell(target, SPELL_BOLT, false);
            m_uiBoltTimer = urand(10000,20000);
        ...
        {
            if(Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
                m_creature->CastSpell(target, SPELL_ROOTS, false);
            m_uiRootsTimer = urand(10000,15000);
        ...
        {
            if(m_creature->getVictim())
                m_creature->CastSpell(m_creature->getVictim(), SPELL_MINI, false);
            m_uiMiniTimer = urand(25000,30000);
        ...
        if(m_uiMushroomsTimer < uiDiff)
        {
            ...
            Creature* Mushroom = m_creature->SummonCreature(ID, m_creature->GetPositionX()+urand(2,25), m_creature->GetPositionY()+urand(2,25), m_creature->GetPositionZ(), m_creature->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000);
...
struct MANGOS_DLL_DECL mob_poisonmushroomAI : public Scripted_NoMovementAI
{
    ...
    void JustDied(Unit* pKiller)
    {
        ...
        m_creature->CastSpell(m_creature, SPELL_POISON, true);
    }
