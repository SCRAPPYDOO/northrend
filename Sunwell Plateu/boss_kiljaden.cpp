...
struct MANGOS_DLL_DECL boss_kiljadenAI : public Scripted_NoMovementAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            //DoCast(m_creature->getVictim(), SPELL_REBIRTH, true);
        ...
        if((OrbTimer < diff) && !BoolOrb)
        {
            ...
            //for(uint i=0; i<k; ++i)
            //{
                Creature* ShieldOrb = m_creature->SummonCreature(ID_DRAGON, m_creature->GetPositionX()+15, m_creature->GetPositionY()+15, m_creature->GetPositionZ(), 0, TEMPSUMMON_CORPSE_DESPAWN, 12000);
            //}
            BoolOrb = true;
        }else OrbTimer -= diff;
        ...
        if (FireBloomCheck < diff)
        {
            if(FireBloomCount < 10)
                for(uint8 i=0; i<5; ++i)
                {
                    if(Unit* FireTarget = Unit::GetUnit(*m_creature, FireBloomTarget[i]))
        ...
        if(SinnisterCastTimer < diff)
        {
            ...
            for(uint8 i=0; i<SinisterCount; ++i)
                if(Unit* Sinister = Unit::GetUnit((*m_creature), SinisterGUID[i][0]))
        ...
        {
            DoScriptText(SAY_KJ_REFLECTION1,m_creature);
        ...
        if(Phase3)
        {
            ...
            {
                //DoCast(m_creature->getVictim(), SPELL_SHADOWSPIKE); NOT WORKING
...
struct MANGOS_DLL_DECL mob_deceiverAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(PortalTimer < diff)
        {
            xx = m_creature->GetPositionX();
            yy = m_creature->GetPositionY();
        ...
        if(ImpTimer < diff)
        {
            if(ImpCount < 4)
            {
                Creature* Imp = m_creature->SummonCreature(ID_IMP, xx, yy, m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 60000);
                if(Imp)
                    if(Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
