...
/* todo
spalsh cos tam
i dot  cos  nie castowany
i lodki sprawdzic  cos nie  castuje
*/
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if(m_uiOrbTargetChanger < uiDiff)
        {
            ...
            if(pSpirit && pSpirit->isAlive())
                if(Unit* pPlayer = SelectTarget(SELECT_TARGET_RANDOM))
        ...
        switch(m_uiCurentBoat)
        {
            case 0: break;
            case 1: 
                if(m_uiAbilityHALDORTimer <= uiDiff)
                {
                    if(m_creature->getVictim())
                        m_creature->CastSpell(m_creature->getVictim(), m_bIsRegularMode ? SPELL_SPIRIT_STRIKE : H_SPELL_SPIRIT_STRIKE, false);
                    m_uiAbilityHALDORTimer = 5000; // overtime
                } else m_uiAbilityHALDORTimer -= uiDiff;
                break;
            case 2: 
                if(m_uiAbilityTORGYNTimer <= uiDiff)
                {
                    for (uint8 i = 0; i < 4; ++i)
                        if (Creature* pSpirit = m_creature->SummonCreature(CREATURE_AVENGING_SPIRIT, m_creature->GetPositionX()+urand(1,10), m_creature->GetPositionY()+urand(1,10), m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
                            if (Unit* pPlayer = SelectUnit(SELECT_TARGET_RANDOM, 0))
                                pSpirit->AI()->AttackStart(pPlayer);
                    m_uiAbilityTORGYNTimer = 15000;
                }else m_uiAbilityTORGYNTimer -= uiDiff;
                break;
            case 3: 
                if(m_uiAbilityRANULFTimer <= uiDiff)
                {
                    m_creature->CastSpell(m_creature, m_bIsRegularMode ? SPELL_SPIRIT_BURST : H_SPELL_SPIRIT_BURST, false);
                    m_uiAbilityRANULFTimer = 10000;
                } else m_uiAbilityRANULFTimer -= uiDiff;
                break;
            case 4:
                if(m_uiAbilityBJORNTimer <= uiDiff)
                {
                    if (Creature* pSpirit = m_creature->SummonCreature(CREATURE_SPIRIT_FOUNT, 385+urand(1,10), -330+urand(1,10), 104.756, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 180000))
                    {
                        pSpirit->SetSpeed(MOVE_RUN, 0.4f);// db suport need
                        ...
                        pSpirit->SetDisplayId(11686);
                        m_uiOrbGUID = pSpirit->GetGUID();
                        m_uiOrbTargetChanger = 0;
                    }
                    m_uiCurentBoat = 0;
        ...
        {
            if(m_creature->getVictim())
                m_creature->CastSpell(m_creature->getVictim(), SPELL_SCREAMS_OF_THE_DEAD, false);

            DoCast(m_creature, SPELL_SCREAMS_OF_THE_DEAD);
            DoCast(m_creature, SPELL_CHANNEL_YMIRON_TO_SPIRIT);

            m_uiCurentBoat = m_uiHealthAmountModifier;

            uint32 ID = 0;
            switch(m_uiCurentBoat)
            {
                case 1: DoScriptText(SAY_SUMMON_BJORN, m_creature);  ID = CREATURE_BJORN_VISUAL;  m_uiAbilityBJORNTimer  = 8000; break;
                case 2: DoScriptText(SAY_SUMMON_HALDOR, m_creature); ID = CREATURE_HALDOR_VISUAL; m_uiAbilityHALDORTimer = 8000; break;
                case 3: DoScriptText(SAY_SUMMON_RANULF, m_creature); ID = CREATURE_RANULF_VISUAL; m_uiAbilityRANULFTimer = 8000; break;
                case 4: DoScriptText(SAY_SUMMON_TORGYN, m_creature); ID = CREATURE_TORGYN_VISUAL; m_uiAbilityTORGYNTimer = 8000; break;
                default: break;
            }

            DoCast(m_creature, SPELL_CHANNEL_YMIRON_TO_SPIRIT);
            if (Creature* pBoat = m_creature->SummonCreature(ID, m_creature->GetPositionX()+urand(1,10), m_creature->GetPositionY()+urand(1,10), m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 15000))
            {
