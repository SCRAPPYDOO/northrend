...
//Achiwement Implementaion from trinity
...
enum
{
    ...
    SPELL_CHARGE_RIFT          = 47747, //Works wrong (affect players, not rifts <- this is visual effect)
    SPELL_CREATE_RIFT          = 47743, //Don't work, using WA
    SPELL_ARCANE_ATTRACTION    = 57063, //Used on Random target when shield is on.
...
struct MANGOS_DLL_DECL boss_anomalusAI : public ScriptedAI
{
    ...
    void JustDied(Unit* pKiller)
    {
		...
		/* ACHIEVEMENT implementation from trinity
		if(m_bIsHeroicMode && !DeadChaoticRift)
        {
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if ((Phase == 0) && (m_creature->GetHealth() <= m_creature->GetMaxHealth() * 0.75))
        {
            ...
            if (Rift)
            {
                ...
                if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0))
				{
					...
					target->CastSpell(target, SPELL_ARCANE_ATTRACTION, true);
				}
        ...
        {
            if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0))
                DoCast(target, m_bIsHeroicMode ? SPELL_SPARK_H : SPELL_SPARK_N);
            SPELL_SPARK_Timer = 5000;
...
struct MANGOS_DLL_DECL mob_chaotic_riftAI : public Scripted_NoMovementAI
{
    ...
    void Reset()
    {
        ...
        DoCast(m_creature, SPELL_ARCANEFORM, true);
    }
    ...
    void UpdateAI(const uint32 diff) 
    {
        ...
        {
            Unit* Anomalus = Unit::GetUnit((*m_creature), m_pInstance->GetData64(DATA_ANOMALUS));
            if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0))
